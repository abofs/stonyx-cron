...
- **Async locking:** State mutations (`add`, `update`, `remove`) and the scheduler's own bookkeeping (the batch claim in `onTimer`, and `#claimJob` / `#settleJob` in `executeJob`) are serialized through `locked()` to prevent race conditions from concurrent async operations. **The consumer callback is deliberately NOT protected:** `executeJob` runs in three phases — claim (locked), invoke (UNLOCKED), settle (locked) — and `await onJobDue(job)` holds no lock at all, because awaiting arbitrary consumer code inside the critical section wedged every subsequent `locked()` call when a callback never settled (#34). Anything designed around the invoke phase (a timeout, a concurrency bound, cancellation) must assume the job map and the heap can be mutated underneath it, and must re-check membership by identity before acting, and must release the claim on any exit that skips settle.
