...
- **`inFlight`, the stale-vs-live claim discriminator (#34):** `src/service.ts` carries a module-level `WeakSet<Job>` — the class's second module-global after `locked()`'s chain, and deliberately NOT instance state. A job object is added when phase 1 claims it and removed by phase 3's `finally` (or by `#executeClaimed`'s one hand-release on the `'removed'` path), so membership means exactly "a settle is still coming". It exists because `start()` cannot otherwise tell a STALE claim from a LIVE one: both are just a number in `job.state.runningAtMs`, and they need opposite treatment. A stale claim (owner process gone) must be released — nothing else ever will, there is no lease on the field — or the job is dead forever while `status()` reports it healthy. A live claim must be left alone, because releasing it lets the timer launch a second concurrent invocation of a job that is already running, breaking the one invariant this class advertises. **Module-level, not per-instance, is the load-bearing choice:** a second `CronService` handed the same live `Job` objects inherits the answer rather than guessing, and identity-keying means a deserialized row is correctly a non-member (a real process restart is what makes a claim provably stale). It adds nothing to the published surface — `grep -c inFlight dist/service.d.ts` is 0 — and needs no `resetLock()`-style test hatch, because fresh per-test job objects cannot be members. **Known limit:** for a HUNG callback the settle is pending, not absent, so membership stays true forever and `start()` will not release it; a batch sibling stranded that way is permanently dead in-process. Object identity cannot distinguish hung-forever from in-flight — that needs a lease on `runningAtMs`, tracked on #35
