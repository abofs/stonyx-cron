...
- **`config.cron.log` is `false` for the whole suite.** `test/config/environment.ts` pins it and the override IS applied — the runner's tsx ESM loader resolves stonyx's `${basePath}.js` config specifier to the `.ts` file. So `service.log()` is a no-op in tests: a test that expects to observe a gated log line must flip `config.cron.log` itself and restore it. Anything routed through the ungated `log.error` is observable by default.
