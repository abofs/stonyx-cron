...
- **`config.cron.log` is `false` for the whole suite.** `test/config/environment.js` pins it and the override IS applied — the declared `stonyx@0.2.3-beta.93` resolves config through an `existsSync` check on the literal `${basePath}.js` path before the dynamic import (verified in the published tarball's `dist/util/import-config.js`), so a `.ts` here is never even looked for. That is why the file must stay `.js`. **Nothing in the suite guards it** — measured: renaming it to `.ts` leaves all 198 tests green, so the rename would land silently. Tracked as #62; `test/unit/publish-surface-test.ts` does NOT cover it (that test asserts on the packaged root `config/environment.js` for #30, a different file in a different directory). So `service.log()` is a no-op in tests: a test that expects to observe a gated log line must flip `config.cron.log` itself and restore it. A test that depends on the flag being *off* must pin it off itself rather than inherit it — another file's unrestored write is not a precondition.
