...
- `findDueJobs` pops items from the heap and `claimJob` detaches them on the `run()` path, so a job is off the heap for the whole unlocked invoke window; only `settleJob` re-inserts it, after updating `nextRunAtMs`. `executeJob` runs settle from a `finally` precisely because a throw that skipped it would leave the job marked running with no heap entry — permanently un-due rather than merely lost for one tick (#34); validation should confirm every exit from the invoke phase reaches a settle
